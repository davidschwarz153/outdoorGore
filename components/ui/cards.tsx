  ...
  return (
    <div className="md:grid md:grid-cols-4 md:gap-10">
      {data.map((el: Product) => {
        return (
          <article
            key={el.id} // Added key prop here
            className="text-emerald-950 h-[40vh] flex flex-col justify-center p-10 border-2 rounded-lg shadow-sm cursor-pointer transform transition-transform duration-200 hover:scale-105"
          >
            <img
              src={el.image}
